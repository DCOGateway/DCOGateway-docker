...
: "${BTCPAYGEN_DOCKER_IMAGE:=dcogateway/docker-compose-generator}"
if [ "$BTCPAYGEN_DOCKER_IMAGE" == "dcogateway/docker-compose-generator:local" ]
then
    ...
    docker rmi $(docker images dcogateway/docker-compose-generator --format "{{.Tag}};{{.ID}}" | grep "^<none>" | cut -f2 -d ';') > /dev/null 2>&1
    set -e
fi
